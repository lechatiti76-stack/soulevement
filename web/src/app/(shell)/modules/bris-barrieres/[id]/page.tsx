...
import { addAnnexe, addComment, deleteAnnexe, getDossier } from "@/modules/bris-barrieres/api";
...
export default function BrisBarrieresDetailPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-semibold">Bris de barrières — {data.dossier.numero}</h1>
      ...
      {PARTS.map((part) => (
        <div key={part} className="space-y-2 rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--bg-elevated))] p-5">
          <h2 className="mb-1 text-sm font-medium">
            {part}. {BRIS_BARRIERES_PART_LABELS[part]}
          </h2>
          {brisBarrieresFieldsForPart(part).map((field) => (
