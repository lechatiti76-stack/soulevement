...
Plateforme de traitement automatique de documents : dépôt (PDF/Word/image) → extraction IA → formulaire dynamique → génération PDF → archivage automatique. Depuis la Phase 8, un dashboard multi-modules donne aussi accès à des formulaires métier dédiés (module "Soulèvement" — fiche wagons — actif ; 3 autres à venir avec le même mécanisme).
...
**Après la mise à jour Phase 8 (module Soulèvement)** : re-exécuter `setupDatabase()` une fois dans l'éditeur Apps Script — ajoute la colonne `module` à `docmod_dossiers` (migration additive, ne touche pas aux données existantes). Sans ça, les dossiers du module "Soulèvement" sont mal aiguillés (numérotation correcte, mais génération PDF et filtrage par module utilisent le comportement par défaut du module "Nouvelle demande") — cf. [ARCHITECTURE.md §13](./ARCHITECTURE.md#13-module-soulèvement-phase-8).
...
### Phase 8 — module "Soulèvement"

Dashboard multi-modules et module métier "Soulèvement" (formulaire 3 parties, OCR par champ, PDF via template Google Slides construit par code) déployés le 2026-07-18. **Vérifié** : les 3 parties du formulaire (navigation, cases à cocher, menus déroulants, signatures multiples), la création/validation d'un dossier via navigateur réel et via appels API directs, la numérotation `SOU-`, les vignettes du dashboard (active vs. grisées), et la non-régression du module "Nouvelle demande". **Pas encore vérifié** : le rendu visuel du PDF généré par le nouveau template Slides — bloqué par une étape manuelle requise (`setupDatabase()` à re-exécuter pour la colonne `module`, cf. ci-dessus) et par l'absence d'accès Drive authentifié depuis l'environnement de développement pour l'ouvrir. Détail complet dans [ARCHITECTURE.md §13](./ARCHITECTURE.md#13-module-soulèvement-phase-8).
