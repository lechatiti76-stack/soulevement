...
- Lancer `npm run build` (production) pendant que `npm run dev` tournait sur le même dossier `.next` a corrompu le serveur de dev (`Cannot find module './276.js'`) — ne pas faire tourner build et dev en parallèle sur le même dossier ; supprimer `.next` et relancer `npm run dev` si ça arrive. Reproduit une seconde fois en Phase 6 (redémarrage du serveur avec un onglet déjà ouvert) — provoque un blocage apparent sur "Chargement..." dans le navigateur ; corrigé par un rechargement complet de la page (pas un bug applicatif, un artefact du hot-reload).
